    ...
    public class StorageClient
    {
        ...
        public byte[] download_file(String group_name, String remote_filename, long file_offset, long download_bytes)
        {
            ...
            try
            {
                ...
                using (var s = new NetworkStream(storageSocket))
                {
                    pkgInfo = ProtoCommon.recvPackage(s, ProtoCommon.STORAGE_PROTO_CMD_RESP, -1);
                                 ...
                                 String local_filename)
        {
            ...
            try
            {
                ...
                var outputStream = new FileStream(local_filename, FileMode.OpenOrCreate, FileAccess.ReadWrite);
                try
                {
                    this.errno = 0;
                    this.send_download_package(group_name, remote_filename, file_offset, download_bytes);

                    using (var inputStream = new NetworkStream(storageSocket))
                    {
                        header = ProtoCommon.recvHeader(inputStream, ProtoCommon.STORAGE_PROTO_CMD_RESP, -1);
         ...
         * @return 0 success, return none zero errno if fail
         */
        public int download_file(String group_name, String remote_filename, IDownloadCallback callback)
        {
            long file_offset = 0;
            long download_bytes = 0;
            return this.download_file(group_name, remote_filename,
         ...
         */
        public NameValueCollection get_metadata(String group_name, String remote_filename)
        {
            bool bNewConnection = this.newUpdatableStorageConnection(group_name, remote_filename);
            Socket storageSocket = this.storageServer.getSocket();
            ...
            try
            {
                ...
                using (var s = new NetworkStream(storageSocket))
                {
                    ...
                    var encoder = Encoding.GetEncoding(ClientGlobal.g_charset);
                    return ProtoCommon.split_metadata(encoder.GetString(pkgInfo.body));
                }
            }
            catch (IOException ex)
            {
                                ...
                                NameValueCollection meta_list, byte op_flag)
        {
            ...
            try
            {
                ...
                {
                    meta_buff = encoder.GetBytes(ProtoCommon.pack_metadata(meta_list));
                }

                filenameBytes = encoder.GetBytes(remote_filename);
                sizeBytes = new byte[2 * ProtoCommon.FDFS_PROTO_PKG_LEN_SIZE];
                sizeBytes.Fill<byte>(0);
                ...
                bs = encoder.GetBytes(group_name);

                groupBytes.Fill<byte>(0);
                ...
                using (var outputStream = new NetworkStream(storageSocket))
                {
                    ...
                    {
                        outputStream.Write(meta_buff, 0, meta_buff.Length);
                    }

                    pkgInfo = ProtoCommon.recvPackage(outputStream, ProtoCommon.STORAGE_PROTO_CMD_RESP, 0);
        ...
        public class UploadBuff : IUploadCallback
        {
             ...
             * @return 0 success, return none zero(errno) if fail
             */
            public int send(Stream outputStream)
            {
